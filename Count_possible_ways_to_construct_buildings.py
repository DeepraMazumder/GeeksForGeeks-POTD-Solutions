...
Explanation: Possible ways for one side are BSS, BSB, SSS, SBS, SSB where B represents a building and S represents an empty space. Pairing up with possibilities on the other side of the road, total possible ways
are 25.
...
class Solution:
    def TotalWays(self, N):
        ...
        for i in range(2,N+1):
            l[i]=(l[i-2]+l[i-1])%(10**9+7)
        r=l[N]+l[N-1]
        return int((r*r)%(10**9+7))
