/*
In a restaurant, two waiters, A and B, receive n orders per day, earning tips as per arrays arr[i] and brr[i] respectively. If A takes the ith order, the tip is arr[i] rupees; if B takes it, the tip is brr[i]
rupees.
...
class Solution 
{
    public:
        long long maxTip(int n, int x, int y, vector<int> &arr, vector<int> &brr) 
        {
            ...
            for(int i=0;i<n;i++) a.push_back({abs(arr[i]-brr[i]), i});
