...
The task is to complete the function isDeadEnd which returns true if the BST contains a dead end else returns false. Here Dead End means a leaf node, at which no other node can be inserted.
...
class Solution:
    ...
    def dead_end(self, root, min_val, max_val):
        if root is None:
            return False
        if min_val== max_val:
            return True
            
        return self.dead_end(root.left, min_val, root.data - 1) or self.dead_end(root.right, root.data +1, max_val)
        
    def isDeadEnd(self, root):
        return self.dead_end(root,1, 10001)
